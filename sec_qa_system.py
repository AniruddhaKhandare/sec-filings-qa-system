...
# Constants (assuming these would be defined in a config or at the top)
cache_dir = "cache"
rate_limit = 0.5
chunk_size = 512
chunk_overlap = 50
...
class EnhancedDocumentProcessor:
                ...
                if matches:
                    ...
                    for other_name, other_pattern in self.section_patterns.items():
                        if other_name != section_name:
                            next_matches = re.search(other_pattern, text[start_pos + 100:],
                                                     re.IGNORECASE)
                            if next_matches:
    ...
    def _extract_entities(self, text: str) -> List[Dict]:
        ...
        if not self.nlp or len(text) > 100000:
            return []
        try:
    ...
    def _calculate_content_quality(self, chunks: List[Dict]) -> float:
        ...
        for chunk in chunks:
            ...
            if hasattr(metadata, 'readability_score') and metadata.readability_score:
                if 30 <= metadata.readability_score <= 60:
                    score += 0.2
...
class PerformanceOptimizer:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        cache_hit_rate = total_cache_hits / (total_cache_hits + total_api_calls) if \
            (total_cache_hits + total_api_calls) > 0 else 0
        return {
...
class ComprehensiveAnswerGenerator:
    ...
    def __init__(self, api_key: str = None):
        ...
        if api_key:
            ...
            logger.warning("No OpenAI API key provided - system will use mock responses")
...
Answer:
    ...
    async def _call_openai_api(self, prompt: str) -> str:
        ...
        try:
            response = await openai.ChatCompletion.acreate(
                ...
                messages=[
                    {"role": "system", "content": "You are a financial research analyst analyzing SEC filings."},
    ...
    def _generate_structured_answer(self, query: str, context_chunks: List[Dict],
        ...
        """Generate structured answer using real LLM"""
        template = self.prompt_templates.get(
            query_context.query_type,
            self.prompt_templates['general']
        )
        ...
        return asyncio.run(self._call_openai_api(prompt))
...
class ComprehensiveEvaluator:
    """Evaluate system against all 10 test questions"""
    def __init__(self, sec_qa_system):
        """TODO: Add docstring."""
        self.system = sec_qa_system
    ...
    def run_all_evaluations(self) -> Dict[str, Any]:
        """Run all 10 evaluation questions"""
        results = []
        for i, question in enumerate(self.evaluation_questions, 1):
            print(f"Processing question {i}/10: {question[:50]}...")
            start_time = time.time()
        ...
        return {
            ...
            'average_processing_time': round(np.mean([r['processing_time'] for r in results]), 2),
...
class SECQACompleteSystem:
    ...
    def __init__(self, config: Dict = None, api_key: str = None):
        ...
        self.answer_generator = ComprehensiveAnswerGenerator(api_key=api_key)
        self.performance_optimizer = PerformanceOptimizer()
        self.evaluator = ComprehensiveEvaluator(self)
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            answer_result = self.answer_generator.generate_answer(query, context_chunks,
                                                                  query_context)
    ...
    def run_full_evaluation(self) -> Dict[str, Any]:
        """Run evaluation against all 10 test questions"""
        print("Starting comprehensive evaluation of all 10 test questions...")
        evaluation_results = self.evaluator.run_all_evaluations()
        performance_report = self.get_performance_report()
        return {
            'evaluation_results': evaluation_results,
            'performance_report': performance_report,
            'system_assessment': {
                'questions_processed': evaluation_results['total_questions'],
                'average_confidence': evaluation_results['average_confidence'],
                'system_ready': evaluation_results['average_confidence'] > 0.6
            }
        }
...
if __name__ == "__main__":
    ...
    try:
        sec_qa_system = SECQACompleteSystem(api_key=api_key)
        ...
        print("Answer:", result['answer'][:500] + "..." if len(result.get('answer', '')) > 500 else result.get('answer', 'No answer'))
        print("Confidence:", result['confidence_analysis']['overall_confidence'])
        print("\n=== RUNNING FULL EVALUATION ===")
        full_evaluation = sec_qa_system.run_full_evaluation()
        print(f"Questions Processed: {full_evaluation['evaluation_results']['total_questions']}/10")
        print(f"Average Confidence: {full_evaluation['evaluation_results']['average_confidence']}")
        print(f"Average Processing Time: {full_evaluation['evaluation_results']['average_processing_time']} seconds")
        print(f"System Ready: {full_evaluation['system_assessment']['system_ready']}")
    except Exception as e:
        print(f"Error: {e}")
        print("Make sure to provide a valid OpenAI API key.")
        print("\nRunning demo with mock responses...")
        demo_system = SECQACompleteSystem()
        demo_result = demo_system.process_query("What are Apple's main risk factors?")
        print("Demo answer:", demo_result['answer'][:200] + "...")
